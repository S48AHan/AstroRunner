...
//heart
int heart=2;
char heartbmp[5][20]={"heart\\heart1.bmp","heart\\heart2.bmp","heart\\heart3.bmp","heart\\heart4.bmp","heart\\heart5.bmp"};
int heartCordinateX=1000;
int	heartCordinateY=140;
...
struct power
{
	...
	int y;
	bool show;
}beam[100],enemy[3],coin[3], rock[3],heart_1[1];
...
void Collision(){
	...
	for(int j=0;j<3;j++){
		if((IMCordinateX+72>=coin[j].x && IMCordinateX<=coin[j].x+60) && (IMCordinateY+IMCordinateJump+98>=coin[j].y && IMCordinateY+IMCordinateJump<=coin[j].y+60)){


			coin[j].x=900+(rand()%200);
			coin[j].y=120+(rand()%50);
			score+=10;
		}
	}
	...
	if((IMCordinateX+72>=heart_1[0].x && IMCordinateX<=heart_1[0].x+60) && (IMCordinateY+IMCordinateJump+98>=heart_1[0].y && IMCordinateY+IMCordinateJump<=heart_1[0].y+30)){


			heart_1[0].x=1200+(rand()%200);
			heart_1[0].y=120+(rand()%50);
...
//high score er jnno box ashbe
void drawTextBox()
{
	iSetColor(255, 255, 255);
	iRectangle(310, 150, 250, 30);
...
void iDraw()
{
	...
	else if(k==4)
	{
		...
		iShowBMP(0,0,"instruction\\galax.bmp");
		iText(5,5,"press B to go to back",(void*)5U);
	}
	...
	iText(255,105,"Use right click to enter your name",(void*)5U);
	iText(5,5,"Use backspace to remove a letter",(void*)5U);
	}

		else if(k==8){

		iShowBMP(0,0,"credit\\creditsfinal.bmp");
		iText(5,5,"press Space to go to back to menu",(void*)5U);
	}

	else if(k==7)
	{
		iShowBMP(0,0,"highscore\\score.bmp");
		iText(5,5,"press G to go to back",(void*)5U);

		iText(350,400,ar[0].name,(void*)5U);
		itoa(ar[0].score,play,10);
		iText(500,400,play,(void*)5U);

		iText(350,350,ar[1].name,(void*)5U);
		itoa(ar[1].score,play,10);
		iText(500,350,play,(void*)5U);

		iText(350,300,ar[2].name,(void*)5U);
		itoa(ar[2].score,play,10);
		iText(500,300,play,(void*)5U);

		iText(350,250,ar[3].name,(void*)5U);
		itoa(ar[3].score,play,10);
		iText(500,250,play,(void*)5U);

		iText(350,200,ar[4].name,(void*)5U);
		itoa(ar[4].score,play,10);
		iText(500,200,play,(void*)5U);
	...
	else if(k==3)
	{
		...
		//heart move show
		iShowBMP2(heart_1[0].x,heart_1[0].y,"heart\\heart1.bmp",0);
	...
	if(heart>=0){
			iSetColor(0,0,0);
			iText(20,300,"Health:");
			iShowBMP2(35,50,heartbmp[heart],0);
...
void iMouse(int button, int state, int mx, int my)
{
	...
	if (button == GLUT_LEFT_BUTTON && state == GLUT_DOWN)
	{
		...
		// menu theke instruction
		
			if((mx>=250  && mx<=627) && (my>=306 && my<=386))
			{

				k=5;
			}
	
		// menu theke high score
		
			if((mx>=250  && mx<=625) && (my>=174 && my<=245))
			{
...
if(k==4)
{
	pauseMenu=true;
	// pause theke resume
	if(mx>=275 && mx<=625 && my>=422 && my<=495)
	{ 
		pauseMenu=false;
		pause=true;
		if(pause){
    ...
    // abar main menu theke new game
	if(mx>=284 && mx<=625 && my>=316 && my<=390)
	{
		k-=2;
		pause=true;
		if(pause){
...
// resume theke instruction
	if((mx>=285  && mx<=627) && (my>=207 && my<=282))
	{
	...
	if (button == GLUT_RIGHT_BUTTON && state == GLUT_DOWN)
	{
		...
		if(mx >= 310 && mx <= 560 && my >= 150 && my <= 180 && mode == 0)
		{
			mode = 1;
		}
	}
...
void iKeyboard(unsigned char key)
{
	...
	if(k==5)
	{
		if(key=='b'&& inMenu==true){

			k=2;
		}
		else if(key=='b' && pauseMenu==true){
...
void heart_set(){
	...
	heart_1[0].x=heartCordinateX;
	heart_1[0].y=120+(rand()%100);
...
void fileread(){
	...
	for(int i=0;i<5;i++){
		...
		fscanf(fptr,"%s %d",&ar[i].name,&ar[i].score);
	}
