...
void AlienFunc(){
	AlienIndex++;
	//enemy.x=AlienCordinateX;
	//enemy.y=AlienCordinateY;
	if (AlienIndex>=8)
		AlienIndex = 1;
}
...
void iDraw()
{
	...
	if(!AlienKill){
		...
		iShowBMP2(enemy[0].x,enemy[0].y,Alien[AlienIndex],0);
	}
...
void Collision(){
	...
	for(int i=0;i<100;i++){
		...
		if((beam[i].x>=enemy[0].x && beam[i].x<=enemy[0].x+72) && (beam[i].y>=enemy[0].y && beam[i].y<=enemy[0].y+98)){
